...
public class VideoMixer2 implements GetUserMediaImpl.CameraSwitchCallback{
    ...
    private final ArrayList<VideoMixerRenderer2> renderers = new ArrayList<>();
    ...
    public VideoMixer2(String name, int framesPerSecond, Size size, int bitrate) {
        this.name = name;
    ...
    private void scheduleFrameGrabber() {
        ...
        renderThreadHandler.postDelayed(
                frameGrabberRunnabele, frameRate);
    }
    ...
    public void addVideoTrack(final VideoTrack track, final boolean local, boolean isMirror, final String label) {
        postToRenderThread(new Runnable() {
            ...
            public void run() {
                ...
                VideoMixerRenderer2 renderer = null;

                for (VideoMixerRenderer2 cRenderer: renderers) {
                ...
                renderer = new VideoMixerRenderer2(label, eglBase, track, local, drawer, renderThreadHandler, handlerLock,statisticsLock);
    ...
    public void removeVideoTrack(VideoTrack track, boolean isLocal, boolean isMirror, String label) {
        postToRenderThread(new Runnable() {
            @Override
            public void run() {
                VideoMixerRenderer2 composeRenderer = null;
    ...
    private void updateLayout() {
        ...
        if(renderers.size() == 1){
            VideoMixerRenderer2 composeRenderer = renderers.get(0);
            VideoMixerRenderer2.LayoutPosition position = new VideoMixerRenderer2.LayoutPosition();
            ...
            for(int i=0; i < renderers.size(); i++){
                VideoMixerRenderer2 composeRenderer = renderers.get(i);
                VideoMixerRenderer2.LayoutPosition position = new VideoMixerRenderer2.LayoutPosition();
                position.height = eglBase.surfaceHeight();
    ...
    private void release() {
        ...
        synchronized (handlerLock) {
            ...
            renderThreadHandler.postAtFrontOfQueue(new Runnable() {
                @Override
                public void run() {
                    for(VideoMixerRenderer2 renderer : renderers) {
                        renderer.release();
                    }
                    renderers.clear();
    ...
    private void renderFrames() {
        ...
        for(VideoMixerRenderer2 renderer : renderers) {
            if (renderer.renderFrameOnRenderThread()) {
                rendered = true;
            }
        }
        ...
        if (rendered) {
            ...
            eglBase.swapBuffers(currentTimeNs);
            synchronized (statisticsLock) {
                ++framesRendered;
                renderSwapBufferTimeNs += (currentTimeNs - swapBuffersStartTimeNs);
                renderTimeNs += (currentTimeNs - startTimeNs);
            }
            if (DEBUG) logD("Drawn frames: "+framesRendered);
            //frameCallback.didCaptureMixedFrame();
        } else {
            synchronized (statisticsLock) {
                ++framesDropped;
                if (DEBUG) logD("Dropped frames: "+framesDropped);
            }
        }
    ...
    public void willSwitchCamera(boolean isFacing, String trackId) {
        postToRenderThread(() -> {
            VideoMixerRenderer2 renderer = getRendererForTrackId(trackId);
            if (renderer == null) {
                return;
            }
    ...
    private VideoMixerRenderer2 getRendererForTrackId(String trackId) {
        for (VideoMixerRenderer2 renderer : renderers) {

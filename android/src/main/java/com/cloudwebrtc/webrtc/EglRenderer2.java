 ...
 */
public class EglRenderer2 implements VideoSink {
    private static final String TAG = "EglRenderer2";
    private static final long LOG_INTERVAL_SEC = 4;

    public interface FrameListener { void onFrame(Bitmap frame); }

    public interface BitmapDataCallback { void onBitmapData(BitmapData frame); }
    ...
    public static class BitmapData {
        ...
        public BitmapData(byte[] buffer, int width,int height) {
            this.buffer = buffer;
            this.width = width;
            this.height = height;
        }
    ...
    private static class HandlerWithExceptionCallback extends Handler {
        ...
        public void dispatchMessage(Message msg) {
            try {
                ...
                Log.e(TAG, "Exception on EglRenderer2 thread", e);
     ...
     */
    public EglRenderer2(String name) {
        this(name, new VideoFrameDrawer());
    }

    public EglRenderer2(String name, VideoFrameDrawer videoFrameDrawer) {
        this.name = name;
    ...
    public void init(@Nullable final EglBase.Context sharedContext, final int[] configAttributes,
        ...
        synchronized (handlerLock) {
            ...
            logD("Initializing EglRenderer2");
            this.drawer = drawer;
            this.usePresentationTimeStamp = usePresentationTimeStamp;

            final HandlerThread renderThread = new HandlerThread(name + "EglRenderer2");
            renderThread.start();
            renderThreadHandler =
     ...
     * the drawer of the EglRenderer2 that was passed in init.
     *
    ...
    private void renderToBitmap(VideoFrame frame) {
        drawMatrix.reset();
        drawMatrix.preTranslate(0.5f, 0.5f);
        drawMatrix.preScale(mirrorHorizontally ? -1f : 1f, mirrorVertically ? -1f : 1f);
    ...
    public void snapshot(final BitmapDataCallback callback) {
        synchronized (handlerLock) {
            if (renderThreadHandler == null) {
                return;
            }
            renderThreadHandler.postAtFrontOfQueue(() -> {
                final int scaledWidth = bitmapTextureFramebuffer.getWidth();
                final int scaledHeight = bitmapTextureFramebuffer.getHeight();
                ...
                callback.onBitmapData(new BitmapData(byteArray, scaledWidth,  scaledHeight));
            });
        }
        return;
    }

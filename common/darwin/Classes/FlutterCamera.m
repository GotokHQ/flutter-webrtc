...
#import "CaptureSession+FlutterDevicePosition.h"
#import "FlutterRTCPixelBufferConverter.h"

const int64_t kNanosecondsPerSecond = 1000000000;

@interface FlutterCamera ()<AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
...
@implementation FlutterCamera {
    ...
    FlutterRTCPixelBufferConverter *_converter;
...
- (instancetype)init {
    return [self initWithDelegate:nil audioEnabled:NO captureSession:[[AVCaptureSession alloc] init]];
}

- (instancetype)initWithDelegate:(__weak id<RTCVideoCapturerDelegate>)delegate audioEnabled:(BOOL)audioEnabled{
    return [self initWithDelegate:delegate audioEnabled:audioEnabled captureSession:[[AVCaptureSession alloc] init]];
}
    ...
    [RTCDispatcher
     ...
     block:^{
        ...
        if (audioEnabled) {
            ...
            if (![self setUpAudioDataOutput]) {
                RTCLogError(@"Failed to setup audio data");
                return;
            }
        } else {
            [self.captureSession removeInput:self->_audioInput];
            [self.captureSession removeOutput:self->_audioDataOutput];
            self->_audioInput = nil;
            self->_audioDataOutput = nil;
        }
...
- (void)startCaptureWithCompletionHandler:(nullable void (^)(NSError *))completionHandler{
    [self startCaptureWithDevice:_currentDevice format:_format fps:_fps completionHandler:completionHandler];
}

- (void)pauseCapture:(BOOL)pause completionHandler:(nullable void (^)(void))completionHandler {
    [RTCDispatcher
     dispatchAsyncOnType:RTCDispatcherTypeCaptureSession
     block:^{
        RTCLogInfo("Pause");
...
- (void)stopRunning:(nullable void (^)(NSError *))completionHandler {
    [RTCDispatcher
     dispatchAsyncOnType:RTCDispatcherTypeCaptureSession
     block:^{
        RTCLogInfo("Stop Running");
...
- (void)startRunning:(nullable void (^)(NSError *))completionHandler {
    [RTCDispatcher
     dispatchAsyncOnType:RTCDispatcherTypeCaptureSession
     block:^{
        NSLog(@"Start Running");
...
- (void)restartCompletionHandler:(nullable void (^)(NSError *))completionHandler {
    [RTCDispatcher
     dispatchAsyncOnType:RTCDispatcherTypeCaptureSession
     block:^{
        RTCLogInfo("Restart");
        ...
        //         self.isRunning = NO;
        //         [self startCaptureWithCompletionHandler:completionHandler];
    }];
}
...
didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer
    ...
    if (captureOutput == _videoDataOutput) {
        ...
        //        NSLog(@"value:%lld", frameTime.value);
        //        NSLog(@"timescale:%lld", frameTime.timescale);
        //        NSLog(@"seconds:%lld", CMTimeGetSeconds(frameTime));
        int64_t timeStampNs = CMTimeGetSeconds(frameTime) * kNanosecondsPerSecond;
        //NSLog(@"timeStampNs:%lld", timeStampNs);
        RTCVideoFrame *videoFrame = [[RTCVideoFrame alloc] initWithBuffer:rtcPixelBuffer
                                                                 rotation:_rotation
                                                              timeStampNs:timeStampNs];
        [self.delegate capturer:self didCaptureVideoFrame:videoFrame];
        //        if (self.audioSamplesInterceptorDelegate) {
        //            [self.audioSamplesInterceptorDelegate didCaptureVideoSamples:pixelBuffer atTime: frameTime rotation:_rotation];
        //        }
    } else {
        if (self.audioSamplesInterceptorDelegate) {
            [self.audioSamplesInterceptorDelegate didCaptureAudioSamples:sampleBuffer];
        }
    }
...
- (dispatch_queue_t)audioQueue {
    if (!_audioQueue) {
        _audioQueue =
        dispatch_queue_create("org.webrtc.cameravideocapturer.audio", DISPATCH_QUEUE_SERIAL);
...
- (BOOL)setUpAudioDataOutput {
    ...
    if (error) {
        NSString* errorDesc = [NSString stringWithFormat:@"Failed to create audio audioInput %d", (int)error.code];
        RTCLogError(@"audio_setup_failed %@", errorDesc);
        return NO;
    }
    if (![_captureSession canAddInput:_audioInput]) {
        RTCLogError(@"audio_setup_failed failed to setup audio");
        _audioInput = nil;
        return NO;
    }
    ...
    if (![_captureSession canAddOutput:_audioDataOutput]) {
        ...
        _audioInput = nil;
        RTCLogError(@"Audio data output unsupported.");
...
- (void)reconfigureCaptureSessionInput {
    ...
    if (@available(iOS 13.0, *)) {
        for (AVCaptureConnection *oldConnection in [_captureSession.connections copy]) {
            [_captureSession removeConnection:oldConnection];
        }
    } else {
        [_captureSession removeConnection:_connection];
    }
    ...
    if (self.audioEnabled && _audioInput && [_captureSession canAddInput:_audioInput]) {
        [_captureSession addInput:_audioInput];
    }
    _connection =
        [AVCaptureConnection connectionWithInputPorts:input.ports
                                               output:_videoDataOutput];
    if (_connection.isVideoStabilizationSupported) {
        _connection.preferredVideoStabilizationMode = AVCaptureVideoStabilizationModeOff;
    }
